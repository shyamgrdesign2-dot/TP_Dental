...
| **Unified hover broadcast — `oral-tags-filter`** | Replaced `oral-tags-show-one` with a single `oral-tags-filter` `CustomEvent`. Detail shapes: `{ all: true }` (card hover) / `{ kind }` (section hover) / `{ kind, name }` (chip hover) / `null` (hide). `OralRecordsList` wires Card / Section / Chip enter+leave (chip leave → restore kind, section leave → restore all, card leave / click → clear). `DentitionView` listens once and applies the filter in its tag render loop |
