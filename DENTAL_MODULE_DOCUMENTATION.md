...
| **Oral Records subsystem** | New region-level oral examination flow (gingiva, mucosa, arches, surfaces). Single "Oral Examination" card in the right column with inline count badge — see `DENTAL_ORAL_EXAMINATION.md` for the full subsystem doc |
| **Dental score gauge removed** | `ScoreCard` / `ScoreTooltip` / `computeDentalScore` / `DIAG_WEIGHT` / `FINDING_WEIGHT` all removed. The right column goes straight from Oral Records → Tooth Records empty-state |
| **Empty Tooth Records state** | When `recordCount === 0`, the records list shows a dashed-border card with a neutral 56px slate tooth icon + helper copy "Click any tooth in the 3D canvas to add findings, procedures and notes." |
| **VoiceRx brand rename** | Right-edge FAB renamed `Dr. Agent` → `VoiceRx` and repositioned to **vertical centre** (was bottom-anchored). Tooltip / aria-label / panel label updated |
| **Tooth assets → WebP** | 32 odontogram tooth PNGs in `/public/teeth/` converted to WebP (sharp `quality:86, effort:6`). **1.1 MB → 81 KB** (92.6% smaller). `FlatDentitionChart` + `DentalChartPrint` now reference `/teeth/<fdi>.webp` with `decoding="async"` (no `loading="lazy"` so they decode inside the offscreen print root) |
| **Print: Plain + Historical via RxPreviewDocument** | `DentalChartPrint.PrintRunner` now renders `<RxPreviewDocument>` for BOTH modes. Plain = empty snapshot + `chartOverride: BLANK_CHART` + `hideOral: true` → 1-page blank odontogram. Historical = `getComposedRxPreviewSnapshot` + filled chart → full Rx paginated across A4 sheets. `FlatDentitionChart` gained `alwaysRender` so the chart shows even when there's no per-tooth data |
| **Print stylesheet** | `@page { size: A4 portrait; margin: 10mm; }`, every `<article>` from `PaginatedRx` becomes its own physical page via `break-after: page`. Letterhead + patient + footer repeat on every sheet |
| **Pagination — measurement-based** | `RxPreviewDocument.PaginatedRx` now uses a hidden replica sheet to measure each block's true height; packs blocks into A4 pages based on actual capacity. Dental Examination section splits into per-tooth blocks (header bundled with first tooth) so teeth flow across sheets instead of clipping |
| **Nested per-tooth dental tables (print)** | Each tooth in the print table view renders as a self-contained card: grey tooth-header row + per-section sub-tables (Past Procedures / Findings / Procedures) with `Name \| Surfaces \| Since \| Notes` columns. Row dividers, gray column-header strip, top stroke per category section |
| **Oral table — by-kind layout** | Oral exam print is NOT mirrored as per-tooth nested tables (oral is recorded by kind, not per tooth). Now: 3 flat tables — Past Procedures / Findings / Procedures — each `Name \| Region \| Since \| Notes`. No per-region grouping or "Right arch / Whole mouth" subheadings |
| **Structured `cols` on RxPreviewLine** | Each preview line carries `cols: { surfaces, since, note }` set at source in `ExaminationTab.toDentalPreviewSections` so the print table can split into proper columns rather than collapsing into `metaParts` strings |
| **Quadrant-split layout retained** | Dentition + scope (oral / full-mouth / arch / quadrant) views use `layoutMode: "split"` — quadrants separated by gaps; maxillary above mandibular with a horizontal gap. Camera auto-fits via `getCameraPreset` (full position + target + fov) on every resize |
| **Camera distance bumps + maxDistance ceiling** | `dentitionZ` / `groupedScopeZ` narrow-aspect buckets bumped so the entire mouth fits at portrait widths without clipping. `OrbitControls.maxDistance` raised to 55–60 (was 22–30) so the camera can actually reach those distances without clamp |
| **Oral SITE cell — wrap + tooltip + width** | `OralPositionCell` trigger: removed `+N` collapse, pills `flex-wrap: wrap` with `rowGap`, all sites visible on multiple lines. Wrapped in `TPTooltip` (arrow, `enterDelay: 250`) listing every full label. SITE column widened (`min-width: 200`, `width: 220`). Trigger button now `min-height: 52px` with `10px 12px` padding so the row grows vertically |
| **Oral SITE dropdown overhaul** | Search-as-you-type input (auto-focused) filters across all groups. 2-column single-view layout (`column-count: 2`) so Distribution + Oral Sites + Tooth Regions + Tooth Surfaces fit without scroll. Item labels 14px, group headings 12px ALL-CAPS. Bottom-of-list fade indicator. Tooth Regions list reordered to put **Full mouth** first |
| **WT (Whole Tooth) annotation** | `Annotations.jsx` `'whole'` zone — was skipped with `continue`; now renders at bbox extreme + 0.55 outward offset → sits **clearly below** maxillary teeth and **above** mandibular teeth. Uses zone color `#34d399`, abbr `WT`, label "Whole Tooth". `Tooth.jsx` already synthesizes findings from `treatmentHistoryDetails` + `toothEntries` so any row with `surfaces: ['whole']` now produces a canvas marker |
| **Oral region tags** | `OralRegionTag3D` pills now match the per-tooth treatment-tag style: `rgba(107,114,128,0.78)` dark slate transparent bg, white text, 4px radius, 10px / 600 — no colour differentiation between dental and oral. Tooltip auto-show on records-card hover **removed** — tooltips only open on direct pill hover. Per-chip hover in the records card fires `oral-tags-show-one` event → DentitionView filters tags to just the hovered entry. Tag anchor offsets reset to small in-bounds nudges so tags land within the dentition frame |
| **Tooltip placement — radiate outward** | `OralRegionTag3D` accepts `tooltipSide: 'above' \| 'below' \| 'left' \| 'right'`. Per-group `GROUP_TIP_SIDE` map: UPPER_ARCH/UR/UL → above, LOWER_ARCH/LR/LL → below, RIGHT_ARCH → left, LEFT_ARCH → right, FULL → below. Each tooltip radiates away from canvas centre so multiple visible tooltips don't overlap |
| **Sticky action-column shadow** | `useStickyActionEdge` hook wired into all 4 exam tables (OralTable, EntryTab, DentalSymptomsBody, PrimaryDiagnosisBody). When `hasOverflow && scrollLeft < maxScroll`, adds `.scrolledEdge` class to `.tableWrap` → CSS descendant selectors apply the linear gradient + box-shadow to `.thSticky`/`.tdStickyAct`. Delete icon: `Trash size:20, strokeWidth:1.5, color: var(--tp-slate-700)` |
| **Print icon** | Stripped to glyph-only — `width:24, height:24, strokeWidth:1.5`, no background, no border, no shadow, no backdrop-blur |
| **Print dropdown** | Wider 320px panel with "PRINT" eyebrow label. Each row: 36×36 violet-tinted icon box + 14px title + 12px sub + chevron-right. Include-patient toggle uses `TPCheckbox` (small) in a footer separated by a top divider |
| **A11y — drawer titles** | `TPDrawerContent` always renders a visually-hidden `SheetTitle` (`sr-only`, defaults to "Drawer", overridable via `title` prop). Fixes Radix `DialogContent requires a DialogTitle` warning on every drawer instance app-wide |
| **Dead code removed** | `DentalChartPrint.jsx`: 305 → 138 lines (−167). Removed all the old hand-rolled chart renderer (`computeZones`, `computeIssues`, `ToothCell`, `ArchRow`, `PatientHeader`, `DentalHistorySections`, `DentalChartSheet` + constants). `ExaminationTab.jsx`: ~170 lines removed (`ScoreCard`, `ScoreTooltip`, `TooltipRow`, `DIAG_WEIGHT`, `FINDING_WEIGHT`, `computeDentalScore`). `RxPreviewDocument.jsx`: −26 lines (`renderDentalSection`, `firstPageDentalBlocks`, `remainingDentalBlocks`) |
| **Onboarding copy** | "Getting Started" 4-step card. Step 1 now covers **both entry points** (3D tooth tap + Oral Examination CTA) so the flow generalises beyond per-tooth charting: (1) Select tooth or Oral Exam, (2) Record findings, (3) Add notes, (4) Review & print. Short headings + ≤2 line subtitles; video covers the rest |
| **Oral Examination card — inline count** | Removed the outer "Oral Records" h3. Count chip (`12px / 700, slate-100 bg, 999px radius`) now sits inline next to the "Oral Examination" title. One card represents the whole section |
| **Oral Notes icon — violet** | Notes row icon recoloured `slate → var(--tp-violet-600)` (background `rgba(164,97,216,0.12)`) so it matches Past Procedures / Findings / Procedures icon treatment |
| **Print Settings — TP blue palette** | PrintSettingsDrawer fully reskinned with `var(--tp-blue-500)` for selected-card border + tint, radio dots, toggles and the **Done** CTA. No more violet for clickable / selected items. Font sizes normalised to even scale (11/11.5/13.5 → 12/14) |
| **Print Settings — Include past visits toggle** | Renamed from "Include historical data" → **"Include past visits"** so the scope reads as dental/oral history (not general medical). `printSettings.includeHistorical` state key preserved. `RxPreviewDocument` reads `settings?.includeHistorical` with a TODO for the composer to merge past-visit dental + oral records (data layer pending) |
| **Preview Rx — Settings gear** | New gear in the Preview drawer header (right edge) opens a 300px popover with two toggles (**Show dental chart** + **Include past visits**). Gear uses the topbar `.toolGreyAlt` style (grey square, no border). State lives in `RxpadHeader` and flows into `<RxPreviewDocument settings={previewSettings}>`. Removed the earlier inline "Current visit only" pill from EndVisitPage |
| **Unified hover broadcast — `oral-tags-filter`** | Replaced `oral-tags-show-one` with a single `oral-tags-filter` `CustomEvent`. Now a **2-level** model (the previous per-section "show only this kind" was removed as noisy): `{ all: true }` for any hover inside the card (card body OR between section chips) / `{ kind, name }` only when the cursor is on a specific chip / `null` on card leave. Chip leave restores `{ all: true }` since the cursor is still inside the card. `DentitionView` listens once and applies the filter in its tag render loop |
| **SITE popover — always-visible scrollbar** | New `.surfacePopoverScroll` SCSS class: `overflow-y: scroll` (not auto) + `scrollbar-gutter: stable` + `scrollbar-color` (Firefox) + 8px webkit scrollbar with slate-400 thumb on slate-100 track. Applied to the popover body so the doctor sees the scroll affordance even before scrolling |
| **Region taxonomy verified** | `ORAL_POSITION_GROUPS` clinically reviewed: Distribution / Oral sites / Tooth regions / Tooth surfaces are all accurate as-is. Source `types.ts` carries a verdict comment explaining each group's clinical match |
| **Patient seed states** | `InitialToothState` interface gained `oralEntries?` + `oralNotes?`. `INITIAL_TOOTH_STATE` now models three personas: **Shyam GR** (`apt-1`) + **Ria Kapoor** (`apt-new`) = empty / first-time (Getting Started card); **Anjali Patel** (`apt-6`) = returning patient with tooth records (RCT+Crown on 36, Filling on 26, Caries on 16, Staining on 47) AND oral entries (past Scaling & Polishing, current Gingivitis + Calculus findings, planned Scaling & Polishing, overall notes) |
| **Inline view — grouped by KIND** | Print/Preview "Inline" view (`OralExamReport` + `dentalToothInlineNode`) reorganised to mirror the table layout. Oral inline now emits one paragraph per kind (Past Procedures / Findings / Procedures), with region + since + note collapsed into a single bracket per item. Dental tooth inline uses `${toothLabel}: ${parts}` (colon, no em dash). All em dashes removed from inline output |
| **TPConfirmDialog molecule** | New shared confirm modal at `components/tp-ui/tp-confirm-dialog.{jsx,module.scss}` modelled on the VoiceRx `ConfirmDialog` molecule. Structure: title + dark × close button → divider → optional yellow warning callout → footer with destructive secondary (red underlined link) + safe primary (TP-blue solid). Supports optional tertiary slot (`tertiaryLabel`/`onTertiary`) for 3-action flows. Exported from `tp-ui/index.ts` |
| **Confirm dialogs everywhere — Rx back / End-Visit edit / all Clear surfaces** | All destructive confirms now use `TPConfirmDialog` with the reference layout. **Rx back arrow** (`RxPadPage`): "Are you sure you want to go back?" with **Discard & Go Back** (red link) + **Save as Draft** (blue solid). **End-Visit back / Edit Digital Rx tile**: "Are you sure you want to edit this?" with **Yes, Edit Rx** (red link) + **No, Stay Here** (blue solid). **Section Clear** (`SectionHeader` dental + `RxPadSection` clinical), **panel-header Clear** (single-tooth + oral exam), and **terminal-diagnosis conflict** ("Mark as <name>?") all migrated. Old `clearDialog*` SCSS classes + `AlertDialog*` imports removed from `ExaminationTab.{jsx,ui.module.scss}` (~94 lines of dead code) |
| **PaginatedRx — stale-pages crash fix** | `RxPreviewDocument.PaginatedRx` re-seeds `pages` synchronously when the block signature (`sig`) changes (e.g. toggling `showDentalChart` off removes the chart block). Render-phase conditional `setPages` + `effectivePages` local + defensive `idxs.filter((i) => blocks[i])` at render time. Fixes `Cannot read properties of undefined (reading 'node')` when toggling settings |
| **VoiceRx FAB — `/rxpad` only** | `DrAgentLayoutShell` no longer mounts `<DrAgentFab>` in `LegacyDrAgentChrome` (appointments / patient-detail / end-visit) or `PushDrAgentChrome` (treatment-plan). Launcher lives only inside `TPRxPadShell` which wraps `/rxpad`. AI sparkle action buttons also stripped from the appointment queue table + calendar event chips in `DrAgentPage.jsx` |
| **MoEngage tracking spec** | New `DENTAL_MOENGAGE_EVENTS.md` — PM-ready event catalogue covering the full Dental Examination journey: 42 events across 9 funnels (Entry & Navigation, Onboarding, 3D Canvas, Single-Tooth, Oral Exam, Records Sidebar, Voice Rx, Preview Rx, End Visit & Print) plus user attributes, cohorts, campaign hooks, and an engineering implementation map. Naming convention `Dental_<Object>_<Verb>`, snake_case property keys, PII rules (never send free-text clinical notes) |
