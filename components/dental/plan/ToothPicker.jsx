...
export function ToothPicker({ value, onChange, isPediatric = false, disabled, variant = "default", onOpenChange, autoOpen = false, expandChips = false }) {
    ...
    const overflowCount = !expandChips && value.length > VISIBLE_CHIPS ? value.length - VISIBLE_CHIPS : 0;
    const overflowTooltipText = overflowCount > 0 ? value.slice(VISIBLE_CHIPS).map(v => `T${v}`).join(", ") : "";
