            ...
            if (typeof ref === "function")
                ...
                createPortal(_jsx("div", { ref: menuRef, className: "overflow-hidden", style: {
                        ...
                        return (_jsxs("button", { type: "button", disabled: action.disabled, onClick: () => {
                                action.onClick?.();
                                setOpen(false);
                            }, className: "flex w-full items-center gap-2.5 px-4 py-2.5 text-left font-['Inter',sans-serif] text-[14px] font-medium text-tp-slate-700 transition-colors hover:bg-tp-slate-50 disabled:cursor-not-allowed disabled:opacity-50", children: [action.icon && _jsx(TPButtonIcon, { size: 20, children: action.icon }), _jsx("span", { className: "flex-1", children: action.label }), action.shortcut ? (_jsx("span", { className: "font-mono text-[12px] text-tp-slate-400", children: action.shortcut })) : (_jsx(ChevronRight, { size: 16, strokeWidth: 1.5, className: "text-tp-slate-400" }))] }, action.id));
                    }) }), document.body)] }));
});
