...
export function DrAgentPanelV0({
  ...
  const patient = useMemo(
    () => (selectedPatientId ? resolveRxContextOption(selectedPatientId) : null),
    [selectedPatientId],
  )

  const summary = useMemo(() => {
    if (!selectedPatientId) return SMART_SUMMARY_BY_CONTEXT[CONTEXT_PATIENT_ID]
    const key = mapUrlPatientIdToAgentDataKey(selectedPatientId)
    return SMART_SUMMARY_BY_CONTEXT[key] || SMART_SUMMARY_BY_CONTEXT[CONTEXT_PATIENT_ID]
  }, [selectedPatientId])
  ...
  }, [selectedPatientId, hasMessagesForPatient, summary, autoMessage, onSnapshotDelivered, hasQuickClinicalSnapshot, patient?.label])
  ...
  }, [inputValue, selectedPatientId, summary, messagesByPatient, phaseByPatient, onSnapshotDelivered, patient?.label])
  ...
  const handleCopy = useCallback((payload: unknown) => {
    ...
    let didCopy = Boolean(p.dentalScanApply)
  ...
  return (
    <div id="dr-agent-panel-root" className="relative flex h-full flex-col bg-white" style={{ minWidth: 350, maxWidth: 400 }}>
      ...
      {hasPatient && (
        <div className={cn("sticky bottom-0 z-10 shrink-0 border-t border-tp-slate-100/80 bg-white shadow-[0_-4px_16px_rgba(15,23,42,0.04)] relative", showContent ? "v0-content-reveal" : "opacity-0")}>
          <div
