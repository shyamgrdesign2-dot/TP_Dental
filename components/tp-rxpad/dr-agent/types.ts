...
export interface VoiceRxSection {
  sectionId: string       // Primary: "symptoms" | "medication" | "investigation" only in `sections`
...
/** Lead card for voice output — same visual role as the dental radiology summary card */
export interface VoiceClinicalExamLead {
  title: string
  subtitle?: string
  summary?: string
...
export interface VoiceStructuredRxData {
  voiceText: string                    // Original transcribed text (not shown in UI; kept for analytics/debug)
  /** Quick-fill sections for RxPad: symptoms, medication, lab investigation only */
  sections: VoiceRxSection[]
  /** Tooth-level dental examination from dictation (same shape as radiology scan rows) */
  dentalTeeth?: DentalScanToothRow[]
  /** Shown above the card body and as the assistant chat line */
  leadInText?: string
  /** Radiology-style header card body (clinical rows + dictation hook) */
  clinicalExam?: VoiceClinicalExamLead
...
// ═══════════════ DENTAL RADIOLOGY SCAN (AI → CHART) ═══════════════

export interface DentalScanFinding {
  zoneId: string
  type: string
  notes?: string
  /** Clinical duration when known, e.g. from chart */
  since?: string
}

export interface DentalScanProcedure {
  name: string
  surface?: string
  date?: string
  status?: string
...
export interface DentalScanHistoryRow {
  name: string
  surface?: string
  since?: string
  notes?: string
...
export interface DentalScanToothRow {
  fdi: string
  implant?: boolean
  diagnoses?: string[]
  findings?: DentalScanFinding[]
  /** Structured treatment lines (preferred for display) */
  treatmentHistoryRows?: DentalScanHistoryRow[]
  /** Structured planned / prior procedures from imaging + chart */
  procedures?: DentalScanProcedure[]
  /** Prior treatment / visit notes — still merged into copy-to-Rx text */
  treatmentHistory?: string[]
  /** Free-text radiology / AI note for this tooth */
  scannerNotes?: string
}
...
export type RxAgentOutput =
  ...
  | { kind: "medical_records_upload_cta"; data: { patientLabel: string } }
  | {
      kind: "dental_radiology_scan"
      data: {
        title: string
        /** Shown under card title — report date only (e.g. "12 April 2026"), no extra labels */
        scanSubtitle?: string
        /** Legacy: modality / source line; optional if scanSubtitle covers it */
        category?: string
        /** Optional narrative body (compact card uses header scanSubtitle + tooth blocks only) */
        summary?: string
...
export interface RxAgentChatMessage {
  ...
  /** @deprecated Prefer `attachments` — kept for older persisted threads */
  attachment?: ChatAttachment
  /** One or more files shown in the user bubble (native multi-upload / multi-select from records) */
  ...
  /** Voice dictation — bubble uses transcript styling */
