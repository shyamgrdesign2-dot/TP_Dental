...
import { DocumentAttachmentsStrip } from "./DocumentAttachmentBubble"
...
function getSourcesForOutput(output?: RxAgentOutput, documents?: PatientDocument[]): SourceEntry[] | null {
  ...
  switch (output.kind) {
      ...
      if (d.keyLabs?.length) {
        ...
        if (prov) {
          ...
          const uploadedDocs = documents?.filter(doc =>
            ["pathology", "radiology", "dental_radiology", "discharge_summary", "prescription", "other"].includes(doc.docType)
          ) || []
    ...
    case "dental_radiology_scan":
      return [{ label: "Records", description: "Dental radiology upload" }]
    ...
    case "voice_structured_rx":
      return [
        { label: "Voice", description: "Clinical dictation transcript" },
        { label: "Dental", description: "Tooth-level examination (FDI)" },
        { label: "EMR", description: "Chart & last-visit context" },
      ]
...
/** Convert light markdown (bold + links) into rich text. Uses non-greedy `**…**` so filenames and phrases render bold. */
function renderLightMarkdown(text: string, onPillTap?: (label: string) => void): React.ReactNode {
  ...
  if (isUser) {
    return (
      <div className="group/msg flex w-full flex-col">
        ...
        <div className="flex w-full max-w-[85%] flex-col items-end gap-[6px]">
          {(() => {
            const chatAttachments =
              message.attachments?.length ? message.attachments : message.attachment ? [message.attachment] : []
            return (chatAttachments.length > 0 || message.text) ? (
            <div className="w-full overflow-hidden rounded-[14px] rounded-br-[4px] bg-tp-slate-200/55 text-[14px] leading-[18px] text-tp-slate-800">
              {chatAttachments.length > 0 ? (
                <div className={cn("p-3", message.text && "pb-2")}>
                  <DocumentAttachmentsStrip attachments={chatAttachments} embedded />
                </div>
              ) : null}
              {message.text ? (
                <div
                  className={cn(
                    ...
                    chatAttachments.length > 0 && "pt-0",
                  )}
                >
                  {message.isVoiceTranscript ? (
                    <div className="flex flex-col gap-1.5 text-left">
                      <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-tp-slate-500">
                        Transcript
                      </span>
                      <span className="text-[14px] leading-[1.55] italic text-tp-slate-700/75">
                        &quot;{message.text}&quot;
                      </span>
                    </div>
                  ) : (
                    renderLightMarkdown(message.text)
                  )}
                </div>
              ) : null}
            </div>
    ...
    <div className="group/msg flex w-full justify-start">
      <div className="flex w-full flex-col items-start">
        ...
        {message.text && (
          <div className="flex items-start gap-[6px]">
            ...
            <p className="text-[14px] leading-[18px] text-tp-slate-700 whitespace-pre-wrap break-words">
              {renderLightMarkdown(shouldStream ? displayText : message.text, onPillTap)}
        ...
        {message.rxOutput && (
          <div
            className={cn(
              "mt-[6px] min-w-0 max-w-full pl-[30px] transition-all duration-[400ms] ease-out",
              cardRevealed
