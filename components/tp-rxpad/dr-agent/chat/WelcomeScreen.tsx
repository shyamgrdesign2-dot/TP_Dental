...
const PATIENT_ACTION_UPLOAD: QuickAction = {
  icon: <DocumentUpload size={ICON_SIZE} variant="Bulk" />,
  title: "Upload reports",
  subtitle: "Select chart records or upload PDF / images to analyze in chat",
  message: "Upload medical records for this patient",
}

function withUploadCapped(actions: QuickAction[]): QuickAction[] {
  const has = actions.some((a) => a.message === PATIENT_ACTION_UPLOAD.message)
  const next = has ? actions : [...actions, PATIENT_ACTION_UPLOAD]
...
function buildPatientActions(summary?: SmartSummaryData): QuickAction[] {
  if (!summary) {
    return withUploadCapped([PATIENT_ACTION_SUMMARY, PATIENT_ACTION_HISTORY, PATIENT_ACTION_VITALS, PATIENT_ACTION_PAST_VISITS])
  }
  ...
  if (hasIntake && specialtyAction) {
    return withUploadCapped([PATIENT_ACTION_INTAKE, PATIENT_ACTION_SUMMARY, PATIENT_ACTION_HISTORY, specialtyAction])
  }
  if (hasIntake && !specialtyAction) {
    return withUploadCapped([PATIENT_ACTION_INTAKE, PATIENT_ACTION_SUMMARY, PATIENT_ACTION_HISTORY, PATIENT_ACTION_VITALS])
  }
  if (!hasIntake && specialtyAction) {
    return withUploadCapped([PATIENT_ACTION_SUMMARY, PATIENT_ACTION_HISTORY, specialtyAction, PATIENT_ACTION_VITALS])
  }
  return withUploadCapped([PATIENT_ACTION_SUMMARY, PATIENT_ACTION_HISTORY, PATIENT_ACTION_VITALS, PATIENT_ACTION_PAST_VISITS])
}
