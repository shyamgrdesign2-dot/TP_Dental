...
export function ActionableTooltip({
  ...
  /** Debounce so touch + synthesized click does not run onAction twice (e.g. Dr Agent copy → chart). */
  ...
  /** Pointer click / synthesized tap — always runs the action (tooltip is secondary). */
  ...
  /* ── Touch-primary: first tap must run the action (not only open the tooltip). ─── */
    ...
    isVisible && typeof document !== "undefined"
      ? createPortal(
          <div
            ...
            style={
              pos
                ? { top: pos.top, left: pos.left, opacity: 1, transition: "opacity 40ms ease-out" }
