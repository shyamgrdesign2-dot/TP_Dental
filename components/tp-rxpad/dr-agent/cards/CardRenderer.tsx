...
/** Render **bold** markdown in text (non-greedy; supports filenames with underscores, etc.) */
...
import { DentalRadiologyScanCard } from "./analysis/DentalRadiologyScanCard"
import { MedicalRecordsUploadCtaCard } from "./action/MedicalRecordsUploadCtaCard"
...
interface CardRendererProps {
  ...
  /** Medical records CTA — opens document picker / bottom sheet */
...
export function CardRenderer({
  ...
  return renderCard(output, onPillTap, onCopy, onSidebarNav, activeSpecialty, onPatientSelect, onOpenMedicalRecords)
}
...
function renderCard(
  ...
  switch (output.kind) {
    ...
    case "dental_radiology_scan":
      return (
        <DentalRadiologyScanCard
          data={output.data}
          onCopy={onCopy as ((payload: import("@/components/tp-rxpad/rxpad-sync-context").RxPadCopyPayload) => void) | undefined}
        />
      )

    case "medical_records_upload_cta":
      return (
        <MedicalRecordsUploadCtaCard
          data={output.data}
          onAddRecords={() => onOpenMedicalRecords?.()}
        />
      )
