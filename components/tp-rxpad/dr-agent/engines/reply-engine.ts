...
import { enrichVoiceStructuredWithPatientContext, VOICE_RX_LEAD_IN } from "./voice-rx-engine"
...
export function buildReply(
  ...
  if (normalized.includes("voice") || normalized.includes("transcript") || normalized.includes("structured rx") || normalized.includes("dictation") || normalized.includes("voice rx")) {
    const voiceText = summary.lastVisit
      ? `Chief complaint ${summary.lastVisit.symptoms}. On general examination ${summary.lastVisit.examination}. Impression ${summary.lastVisit.diagnosis}. Medication ${summary.lastVisit.medication}. Suggest labs ${summary.lastVisit.labTestsSuggested}. Dental examination dictation. Tooth number 22 upper left lateral incisor: endosseous implant with crown; patient states placement in August 2022; light plaque and stain buccally; occlusion feels stable. Tooth number 24 upper left first premolar: cavitated caries on lateral buccal surface toward contact; resin sealant occlusal since 2016 with partial mesial loss; cold test shows delayed response versus adjacent.`
      : "Chief complaint sore throat and low-grade fever since two days. Prescribing paracetamol 650 milligram one zero one after food for five days and warm saline gargle. Order CBC and CRP if fever persists beyond forty-eight hours. Dental examination dictation. Tooth number 22 upper left lateral incisor: endosseous implant and crown; patient says it was placed in August 2022; buccal plaque noted; no mobility reported. Tooth number 24 upper left first premolar: cavity on lateral buccal surface near contact; old sealant 2016 with partial loss; cold test delayed."
    const data = enrichVoiceStructuredWithPatientContext(voiceText, summary)
    return {
      text: data.leadInText ?? VOICE_RX_LEAD_IN,
      rxOutput: {
...
type DocType = "pathology" | "radiology" | "prescription" | "dental_radiology" | "generic"
...
function buildDentalRadiologyReply(): ReplyResult {
  const teeth = [
    {
      fdi: "26",
      implant: true,
      ...
      treatmentHistoryRows: [
        { name: "Implant-supported crown", surface: "Whole tooth", since: "Aug 2024" },
      ],
      findings: [{ zoneId: "cervical", type: "Calculus", notes: "Supracrestal density — peri-implant hygiene" }],
      scannerNotes:
        "Fixture appears centred in the 26 site; crestal bone height within common limits on this panoramic projection. No obvious thread exposure. Compare with baseline periapical if the patient reports pain or bleeding on probing.",
    },
    {
      fdi: "37",
      implant: false,
      diagnoses: [] as string[],
      treatmentHistoryRows: [
        { name: "Prior episodes of tenderness", surface: "Whole tooth", notes: "Analgesia as needed" },
      ],
      findings: [
        { zoneId: "root", type: "Sensitivity", notes: "Periapical translucency vs widened PDL — vitality and percussion" },
        { zoneId: "occlusal", type: "Cavity/Caries", notes: "Deep radiolucency — bitewing / PA for pulp proximity" },
      ],
      scannerNotes:
        "If symptoms persist, rule out vertical root fracture; CBCT may be indicated beyond this OPG.",
    },
  ]
  return {
    text: "Here’s a quick reconstruction from your OPG — the priority sites are expanded in the cards.",
    rxOutput: {
      kind: "dental_radiology_scan",
      data: {
        title: "Dental radiology — panoramic (OPG)",
        scanSubtitle: "12 April 2026",
        category: "",
        fileLabel: "OPG upload",
        summary:
          "Brief OPG reconstruction: 26 — implant crown with peri-implant calculus; 37 — periapical change plus occlusal radiolucency. Findings and notes are grouped by tooth below.",
        teeth,
      },
    },
  }
...
export function buildDocumentReply(
  ...
  switch (docType) {
    ...
    case "dental_radiology":
      return buildDentalRadiologyReply()
    default:
